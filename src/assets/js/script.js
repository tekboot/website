...
$("#feeling-btn").on("click", function () {
  ...
  var feeling_activity = $(
    "#additional-input .feeling-input .form-control"
  ).val();
});
...
$(".comment-click").on("click", function () {
  ...
  $(this)
    .parents(".post-details")
    .find(".comment-section .comments")
    .toggleClass("d-block");
});
...
$(document).mouseup(function (e) {
  ...
  if (!post.is(e.target) && post.has(e.target).length === 0) {
    $(".create-post").removeClass("active");
  ...
  // react panel
  var react = $(".react-btn");
  if (!react.is(e.target) && react.has(e.target).length === 0) {
